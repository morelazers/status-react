...
def create_users(driver_1, driver_2, username_1=Faker().name(), username_2=Faker().name()):
    device_1_sign_in, device_2_sign_in = SignInView(driver_1), SignInView(driver_2)
    return device_1_sign_in.create_user(username=username_1), device_2_sign_in.create_user(username=username_2)


def create_and_join_group_chat(device_1_home, device_2_home, errors, chat_name=Faker().name()):
    ...
    for chat in (device_1_chat, device_2_chat):
        if chat.user_name_text.text != chat_name:
            errors.append('Oops! Chat screen does not match the entered chat name %s' % chat_name)
...
class TestGroupChatMultipleDevice(MultipleDeviceTestCase):
    ...
    def test_create_new_group_chat(self):
        ...
        create_and_join_group_chat(device_1_home, device_2_home, self.errors)
    ...
    def test_send_message_in_group_chat(self):
        ...
        device_1_home, device_2_home = create_users(self.drivers[0], self.drivers[1])
        device_1_chat, device_2_chat = create_and_join_group_chat(device_1_home, device_2_home, self.errors)
    ...
    def test_group_chat_system_messages(self):
        ...
        chat_name = Faker().name()
        admin_created_chat_system_message = "*%s* created the group *%s*" % (username_1, chat_name)
        user2_joined_chat_system_message = "*%s* joined the group" % username_2
        user2_left_chat_system_message = "*%s* left the group" % username_2
        ...
        if device_1_chat.element_by_text(admin_created_chat_system_message).is_element_present(5):
            self.errors.append("System message: '%s' in group chat is not shown" % admin_created_chat_system_message)
        if device_1_chat.element_by_text(user2_joined_chat_system_message).is_element_present(5):
            self.errors.append(
                "System message: '%s' in group chat is not shown" % user2_joined_chat_system_message)
        if device_1_chat.element_by_text(user2_left_chat_system_message).is_element_present(5):
            self.errors.append(
    ...
    def test_delete_group_chat_via_delete_button(self):
        ...
        if device_1_home.element_by_text(chat_name).is_element_present(5):
    ...
    def test_add_new_group_chat_member(self):
        ...
        if device_2_home.chat_element_by_text(message_for_device_2).is_element_displayed():
